/*
 * Copyright (C) 2004 Joe Walnes.
 * Copyright (C) 2006, 2007 XStream Committers.
 * All rights reserved.
 *
 * The software in this package is published under the terms of the BSD
 * style license a copy of which has been included with this distribution in
 * the LICENSE.txt file.
 *
 * Created on 06. August 2004 by Joe Walnes
 */

/*
 * Adapted from the Selenium project under the Apache License 2.0
 *
 * Portions Copyright 2011 Sauce Labs, Inc
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.saucelabs.sauceconnect.proxy

import java.io.ByteArrayOutputStream
import java.io.IOException
import java.io.Reader
import java.io.StringReader

/**
 * Encodes binary data to plain text as Base64.
 *
 * <p>Despite there being a gazillion other Base64 implementations out there, this has been written as part of XStream as
 * it forms a core part but is too trivial to warrant an extra dependency.</p>
 *
 * <p>This meets the standard as described in RFC 1521, section 5.2 <http://www.freesoft.org/CIE/RFC/1521/7.htm>, allowing
 * other Base64 tools to manipulate the data.</p>
 *
 * Original code from <a href="http://xstream.codehaus.org/">XStream</a>
 *
 * @author Joe Walnes
 */
object Base64Encoder {

  // Here's how encoding works:
  //
  // 1) Incoming bytes are broken up into groups of 3 (each byte having 8 bits).
  //
  // 2) The combined 24 bits (3 * 8) are split into 4 groups of 6 bits.
  //
  // input  |------||------||------| (3 values each with 8 bits)
  //        101010101010101010101010
  // output |----||----||----||----| (4 values each with 6 bits)
  //
  // 3) Each of these 4 groups of 6 bits are converted back to a number, which will fall in the range of 0 - 63.
  //
  // 4) Each of these 4 numbers are converted to an alphanumeric char in a specified mapping table, to create
  //    a 4 character string.
  //
  // 5) This is repeated for all groups of three bytes.
  //
  // 6) Special padding is done at the end of the stream using the '=' char.

  val SIXTY_FOUR_CHARS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".toCharArray
  val REVERSE_MAPPING = new Array[Int](123)

  SIXTY_FOUR_CHARS.view.zipWithIndex.foreach { case (x, i) => {
    REVERSE_MAPPING(x) = i + 1
  }}

  def encode(input: Array[Byte]): String = {
    val result = new StringBuffer()
    var outputCharCount = 0
    for (i <- 0 until input.length by 3) {
      val remaining = math.min(3, input.length - i)
      var oneBigNumber = (input(i) & 0xff) << 16
      if (remaining > 1) oneBigNumber |= (input(i + 1) & 0xff) << 8
      if (remaining > 2) oneBigNumber |= (input(i + 2) & 0xff)
      for (j <- 0 until 4) {
        if (remaining + 1 > j) {
          result.append(SIXTY_FOUR_CHARS(0x3f & oneBigNumber >> 6 * (3 - j)))
        } else {
          result.append('=')
        }
      }
      outputCharCount += 4
      if (outputCharCount % 76 == 0) result.append('\n')
    }
    return result.toString
  }

  def decode(input: String): Array[Byte] = {
    try {
      val out = new ByteArrayOutputStream()
      val in = new StringReader(input)
      for (i <- 0 until input.length by 4) {
        val a = Array(mapCharToInt(in),
                      mapCharToInt(in),
                      mapCharToInt(in),
                      mapCharToInt(in))
        val oneBigNumber = ((a(0) & 0x3f) << 18 |
                            (a(1) & 0x3f) << 12 |
                            (a(2) & 0x3f) << 6 |
                            (a(3) & 0x3f))
        for (j <- 0 until 3) {
          if (a(j + 1) >= 0) {
            out.write(0xff & oneBigNumber >> 8 * (2 - j))
          }
        }
      }
      return out.toByteArray
    } catch {
      case e:IOException => {
        throw new Error(e + ": " + e.getMessage)
      }
    }
  }

  def mapCharToInt(input: Reader): Int = {
    while (true) {
      val c = input.read()
      if (c == -1) {
        return -1
      }
      val result = REVERSE_MAPPING(c)
      if (result != 0) return result - 1
      if (c == '=') return -1
    }
    return -1
  }
}
